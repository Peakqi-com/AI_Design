...
const createSampleProject = (): CrmProject => {
  ...
  return {
    ...
    note: "這是一筆範例資料。你可以在「新增婚禮專案」建立自己的案件並同步註記到 CRM。",
    quotationItems: [
      { id: createId("quote"), name: "婚禮統籌服務費", quantity: 1, unitPrice: 68000 },
      { id: createId("quote"), name: "婚禮當日場控執行", quantity: 1, unitPrice: 18000 },
    ],
    workflowTasks: [
      {
        id: createId("task"),
    ...
    auspiciousPlan: {
      ...
      recommendations: ["新人進場可安排在 11:30-12:00，便於午宴銜接。"],
      generatedAt: now,
    },
    createdAt: now,
    updatedAt: now,
...
const normalizeDressSelectionRecords = (
  ...
  return items
    .map((item) => ({
      ...
      dressName: item.dressName?.trim() || "未命名婚紗",
